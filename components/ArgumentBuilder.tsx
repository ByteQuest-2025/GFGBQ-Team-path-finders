...
import type { ArgumentBuilderResult, TrustedOutput } from '../types';
...
export const ArgumentBuilder: React.FC = () => {
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            ...
            // Verify the precedents cited in the argument
            const precedentsText = response.suggested_precedents.join('. ');
            const verificationResult = await verifyLegalText(precedentsText);
            setVerification({
                originalText: precedentsText,
                    ...
                    <div className="h-[calc(100vh - 200px)] overflow-y-auto pr-2">
                        ...
                        {result && (
                            <div className="space-y-6">
                                {/* Precedent Verification Alert */}
                                {verification && (
                                    <div className="bg-purple-50 border-l-4 border-purple-500 p-4 rounded-r-lg">
                                        <div className="flex items-center justify-between mb-2">
                                            <h4 className="text-sm font-semibold text-purple-900">Precedent Verification</h4>
                                        ...
                                        <p className="text-xs text-purple-700 mt-2">
                                            {verification.overallTrustScore >= 0.7
                                                ? '✅ Suggested precedents have been verified.'
                                                : verification.overallTrustScore >= 0.4
                                                ? '⚠️ Some precedents may be AI-generated. Cross-verify with legal databases.'
                                                : '❌ Warning: Precedents appear to be AI-generated. Verify independently before citing.'}
                                        </p>
                                    </div>
                                )}
