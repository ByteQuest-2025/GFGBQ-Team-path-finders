...
type JudgeView = 'explainability' | 'biasmonitor' | 'hallucination';
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="xl:col-span-2">
                 <div className="bg-[rgb(var(--card))] p-6 rounded-lg shadow-md h-full flex flex-col">
                    {FEATURE_FLAGS.aiBiasMonitor && (
                         <div className="border-b border-[rgb(var(--border))] mb-4">
                            <nav className="-mb-px flex space-x-6">
                                ...
                                <button onClick={() => setActiveView('hallucination')} className={`whitespace-nowrap pb-3 px-1 border-b-2 font-medium text-sm flex items-center gap-2 ${activeView === 'hallucination' ? 'border-[rgb(var(--primary))] text-[rgb(var(--primary))]' : 'border-transparent text-[rgb(var(--muted-foreground))] hover:text-[rgb(var(--foreground))]'}`}>
                    ...
                    {activeView === 'hallucination' && <HallucinationDetector />}
