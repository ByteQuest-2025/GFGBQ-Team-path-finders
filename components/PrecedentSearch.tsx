...
import type { RAGResult, TrustedOutput } from '../types';
import { Spinner } from './Spinner';
import { UploadIcon } from './icons/UploadIcon';
import { FileTextIcon } from './icons/FileTextIcon';
import { SearchIcon } from './icons/SearchIcon';
import { useTranslations } from '../hooks/useTranslations';
import { TrustBadge, TrustScoreBar, VerificationStatus } from './TrustBadge';
...
export const PrecedentSearch: React.FC = () => {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      // Verify the AI-generated answer
      const verificationResult = await verifyLegalText(searchResult.answer);
      setVerification({
        originalText: searchResult.answer,
        ...
        <div className="bg-[rgb(var(--card))] p-6 rounded-lg shadow-md border border-[rgb(var(--border))]">
          ...
          {result && (
            <div className="space-y-6">
              {/* Trust Score */}
              {verification && (
                <div className="bg-white/50 p-4 rounded-lg border border-[rgb(var(--border))]">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-sm font-semibold text-[rgb(var(--card-foreground))]">AI Output Verification</h4>
                    <TrustBadge trustScore={verification.overallTrustScore} size="medium" showPercentage />
                  </div>
                  <TrustScoreBar trustScore={verification.overallTrustScore} showLabel={false} />
                  <p className="text-xs text-[rgb(var(--muted-foreground))] mt-2">
                    This answer has been automatically verified for accuracy and hallucinations.
                  </p>
                </div>
              )}
              ...
              {verification && verification.verificationResults.length > 0 && (
                <div>
                  <h4 className="text-lg font-bold text-[rgb(var(--card-foreground))] mb-3">Claim Verification</h4>
                  <div className="space-y-2">
                    {verification.verificationResults.slice(0, 3).map((vr, index) => (
                      <div key={index} className="bg-white/30 p-3 rounded border border-[rgb(var(--border))] text-sm">
                        <div className="flex items-start justify-between mb-1">
                          <VerificationStatus status={vr.status} confidence={vr.confidence} size="small" />
                        </div>
                        <p className="text-xs text-[rgb(var(--muted-foreground))] mt-1">{vr.claim}</p>
                      </div>
                    ))}
                  </div>
                </div>
