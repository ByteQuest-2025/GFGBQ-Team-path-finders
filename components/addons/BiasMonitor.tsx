...
export const BiasMonitor: React.FC = () => {
    ...
    return (
        <div className="h-full flex flex-col">
            <h2 className="text-xl font-semibold mb-4 text-[rgb(var(--card-foreground))]">Bias & Hallucination Monitor</h2>
            <div className="flex-shrink-0">
                <p className="text-sm text-[rgb(var(--muted-foreground))] mb-2">
                    Analyzes text for potential biases (gender, confirmation, etc.) AND AI hallucinations (fake citations, invalid statutes, unsupported claims)
                </p>
                <textarea
                    value={documentText}
                    ...
                    <div>
                        {result.has_bias === false ? (
                             <div className="bg-green-500/10 border-l-4 border-green-500 text-green-700 p-4 rounded-r-lg" role="alert">
                                ...
                                <p>The text appears to be free from bias and hallucinations. All claims appear verifiable.</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                <div className="bg-amber-500/10 border-l-4 border-amber-500 text-amber-700 p-4 rounded-r-lg" role="alert">
                                    <p className="font-bold">⚠️ Issues Detected</p>
                                    <p>The following biases or potential hallucinations were detected in the text:</p>
                                </div>
